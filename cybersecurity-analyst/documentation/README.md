# 📚 Documentation — Cybersecurity Knowledge Base

Bem-vindo ao meu repositório de documentação de Cibersegurança.
Este espaço foi criado para centralizar anotações, procedimentos, pesquisas, comandos, troubleshooting, estudos e referências utilizadas no dia a dia da área de Segurança da Informação.

O objetivo é manter uma base organizada, rápida de consultar e continuamente atualizada para apoio em atividades técnicas, laboratoriais e profissionais.

**🎯 Objetivo**
Esta pasta serve como uma biblioteca técnica pessoal para:

Consulta rápida de comandos e procedimentos

Registro de análises e investigações

Estudos de ferramentas de segurança

Documentação de incidentes e troubleshooting

Cheatsheets e anotações importantes

Resumos de laboratórios e certificações

Referências úteis para Blue Team, SOC, DFIR e Pentest

**🛠️ Conteúdos Documentados
🔵 Blue Team**

SIEM

Wazuh

Microsoft Sentinel

Elastic Stack

Logs e correlação de eventos

Detecção de ameaças

IOC e hunting

**🔴 Red Team / Pentest**

Reconhecimento

Enumeração

Exploração

Pós-exploração

Ferramentas ofensivas

Web Security

**🟣 DFIR / Forense**

Investigação de incidentes

Coleta de evidências

Timeline analysis

Análise de logs

Malware analysis

**🌐 Redes & Infraestrutura**

TCP/IP

Modelo OSI

VLAN

DNS

VPN

Firewall

Proxy

Hardening

**☁️ Cloud Security**

Azure

IAM

Key Vault

Defender

Sentinel

Segurança em nuvem

**🐧 Linux & Windows**

Administração

Segurança

Scripts

PowerShell

Bash

Troubleshooting

**🧠 Filosofia**
“Documentar economiza tempo, reduz erros e transforma experiência em conhecimento reutilizável.”

**Toda documentação armazenada aqui busca:**

Clareza

Organização

Consulta rápida

Reprodutibilidade

Aprendizado contínuo


**🔍 Ferramentas Frequentemente Utilizadas**

- Wazuh

- Microsoft Sentinel

- Elastic Stack

- Wireshark

- Metasploit

- Nmap

- Burp Suite

- Splunk

**🚀 Evolução Contínua**

Este repositório está em constante atualização conforme:

novos estudos,

novas ferramentas,

experiências práticas,

laboratórios,

certificações,

pesquisas e investigações.

**👨‍💻 Autor**
Paulo Cesar Security
Cybersecurity • SOC • Pentest • Forense • OSINT
